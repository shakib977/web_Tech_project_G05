...
<div class="grid-2">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="index.php?page=instructor&action=upload_material" enctype="multipart/form-data">
                <input type="hidden" name="course_id" value="<?= $course['id'] ?>">
                <div class="form-group">
                    <label class="form-label">Title <span class="required">*</span></label>
                    <input type="text" name="title" class="form-control" placeholder="e.g. Lecture 3 Notes" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Type</label>
                    <select name="material_type" id="mat_type" class="form-control" onchange="toggleMaterialInput(this.value)">
    ...
    <!-- Materials List -->
    <div class="card">
        <div class="card-header"><span class="card-title">📁 Materials (<?= count($materials) ?>)</span></div>
        <div class="card-body" style="padding:0">
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:13px 20px;border-bottom:1px solid var(--border)">
                ...
                <div style="flex:1">
                    ...
                    <div class="text-xs text-muted"><?= ucfirst($m['material_type']) ?> • <?= date('M d', strtotime($m['created_at'])) ?></div>
                </div>
                <a href="index.php?page=instructor&action=delete_material&mat_id=<?= $m['id'] ?>&course_id=<?= $course['id'] ?>"
