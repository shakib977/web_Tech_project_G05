...
<div class="flex-between mb-4" style="margin-bottom:16px">
    <div>
        <h3 class="fw-bold"><?= htmlspecialchars($course['title']) ?> — Q&A Board</h3>
...
<?php if (empty($questions)): ?>
    <div class="empty-state card" style="padding:60px">
        <div class="empty-icon">❓</div><h3>No questions yet</h3><p>Students haven't asked any questions.</p>
    </div>
<?php else: ?>
...
<div class="qa-item">
    <div class="flex-between">
        ...
        <?php if (!$q['is_resolved']): ?>
        <a href="index.php?page=instructor&action=resolve_question&q_id=<?= $q['id'] ?>&course_id=<?= $course['id'] ?>"
           class="btn btn-success btn-sm">Mark Resolved</a>
        <?php endif; ?>
    </div>
    ...
    <div class="qa-answer <?= $a['is_endorsed']?'endorsed':'' ?>">
        <div class="flex-between mb-2" style="margin-bottom:6px">
            ...
            <?php if (!$a['is_endorsed']): ?>
            <a href="index.php?page=instructor&action=endorse_answer&ans_id=<?= $a['id'] ?>&course_id=<?= $course['id'] ?>"
               class="btn btn-outline btn-sm">⭐ Endorse</a>
            <?php endif; ?>
        </div>
    ...
    <!-- Reply Form -->
    <form method="POST" action="index.php?page=instructor&action=answer_question" style="margin-top:12px">
        <input type="hidden" name="qa_question_id" value="<?= $q['id'] ?>">
        <input type="hidden" name="course_id"      value="<?= $course['id'] ?>">
        <div class="flex gap-2">
            <input type="text" name="body" class="form-control" placeholder="Write an answer..." required>
            <button type="submit" class="btn btn-primary" style="white-space:nowrap">Post Answer</button>
        </div>
    </form>
</div>
<?php endforeach; ?>
